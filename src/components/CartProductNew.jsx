...
function CartProductNew(props) {
  const cart = useContext(CartContextNew);
  const id = props.id;
  const quantity = props.quantity;
  const productDataNew = getProductDataNew(id);
        ...
        <li className="flex flex-col py-6 sm:flex-row sm:justify-between">
          <div className="flex w-full space-x-2 sm:space-x-4">
            <img
              ...
              src={productDataNew.img}
              alt="Polaroid camera"
            />
            <div className="flex flex-col justify-between w-full pb-4">
              <div className="flex justify-between w-full pb-2 space-x-2">
                <div className="space-y-1">
                  <h3 className="font-bold">{productDataNew.title}</h3>
                  <h3 className="text-xs">{productDataNew.subTitle}</h3>
                </div>
                <div className="text-right">
                  <p>{quantity} total</p>
                  <p>${(quantity * productDataNew.price).toFixed(2)}</p>
                </div>
              </div>
